...
Run `node --version` and `node <checkout>/src/cli.mjs doctor`. Doctor does not launch the worker. For a runtime check, import `PdfEditor` and `version`, call `await PdfEditor.create()`, compare `editor.engineInfo.version` with `version`, then close in `finally`. Creation rejects JS/native mismatch. Do not claim runtime verification merely from existing files.
