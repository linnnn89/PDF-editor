...
3. Follow [operations](references/operations.md) for inspection, selective fields, encoding diagnostics, exact replacement plans, optional text-bound validation, receipts, and output previews. Its final sections route crop, composition, and the narrow year-label example.
...
For encoding failures, inspect `editReasonCode` and `editReason` before planning
an alternative. Preserve exact Unicode, especially mathematical minus versus
hyphen. Check the bundled API documentation for capabilities; the version string
alone does not distinguish an older release from a newer source-built package.
